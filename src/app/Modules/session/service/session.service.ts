...
export class SessionService {
  ...
  resetPassword(post): Observable<Session> {
    return this.apiService.posts(`employee/change-password`, post).pipe(map(response => (response)));
  }
  confirmPassword(obj): Observable<Session> {
    return this.apiService.forgot(`employee/forgot-password`, obj).pipe(map(response => (response)));
  }
