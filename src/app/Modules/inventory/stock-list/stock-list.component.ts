...
export class StockListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'stockItemName', 'inventoryTypeName', 'unitTypeName', 'stockItemCode', 'activeStatus'];
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            this.getCheckedValue().forEach(item => {
              stockItemId.push(item.inventoryStockItemId)
            })
            let obj = {
              'deletedId': stockItemId,
              'customerId': this.customerId,
            }
