...
export class AttendanceTimeComponent implements OnInit {
  ...
  this.getCheckedValue().forEach(item => { empAttendanceGroupId.push(item.empAttendanceGroupId)
...
saveStatus(activeStatus) {
  ...
  this.getCheckedValue().forEach(item => { ids.push(item.empAttendanceGroupId)
    this.empAttendanceGroupId=item.empAttendanceGroupId 
})
  let obj = {
