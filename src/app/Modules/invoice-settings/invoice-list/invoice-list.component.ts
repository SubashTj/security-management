...
export class InvoiceListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'referenceNo','epf','esi','mode','cgst','sgct','bankGstNo','activeStatus'];
  ...
  getCheckedValue(): Invoice[] {
    let transList: Invoice[] = [];
  ...
  checkboxLabel(row?: Invoice): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${this.referenceNo + 1}`;
  }
  constructor(public config: ConfigService,public dialog: MatDialog,private getService:InvoiceSettingsService,private loader:AppLoaderService) { 
    this.config.init();
  ...
  getData(){
    ...
    this.getService.getInvoice(obj).subscribe((data) => {
      this.paginationDTO.totalSize = data.pageSize;
      this.currentPageDataSize = data.responseModel.length;
      this.dataSource = new MatTableDataSource<Invoice>(data.responseModel);
      this.dataSource.sort = this.sort;
    })
  }
  addSub() {
  ...
  action(option) {
    if (this.getCheckedValue().length == 1) {
      ...
      Swal.fire({
        ...
        text: "Please Select One Invoice!",
      });
    }
}
Delete(option){
...
let referenceNo = [];
this.getCheckedValue().forEach(item => { referenceNo.push(item.referenceNo)
})
let obj={
  'customerId': this.customerId,
...
this.getService.Delete(obj).subscribe((data) => {
  Swal.fire({
    icon: "success",
    title: "Deleted Invoice Successfull",
    timer: 2500
  }).then(function(){
  ...
  if (this.getCheckedValue().length >= 1) {
    ...
    if (activeStatus == 2) {
      ...
      Swal.fire({
        ...
        text: 'Invoice registration incompleted so can not ' + lblAction + '!',
      })
    }
      ...
      Swal.fire({
        icon: 'error',
        title: 'Invoice Status',
        text: 'Please select the ' + opplblAction + ' status of the Invoice!',
      });
    }
  } else {
    Swal.fire({
      icon: 'error',
      title: activeStatus == 2 ? 'Delete Invoice' : 'Change Invoice Status',
      text: 'Please Select Altleast One Invoice!',
    });
  }
}
...
saveStatus(activeStatus) {
  let ids = [];
  this.getCheckedValue().forEach(item => { ids.push(item.referenceNo)
  ...
  this.getService.status(obj).subscribe((res) => {
    if (res.statusCode == 200) {
      Swal.fire({
        icon: "success",
        title: " Invoice Status Changed Successfull",
        timer: 2500
      }).then(function(){
        window.location.reload();
    });
      ...
      Swal.fire({
        icon: "error",
        title: " Invoice Status Change Failed",
        timer: 2500
