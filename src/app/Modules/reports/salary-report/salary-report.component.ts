...
export class SalaryReportComponent implements OnInit {
  ...
  districtId: string;
  responseModel: any
  displayedColumns2: string[] = ['employeeName', 'employeeId', 'finalSalary', 'presentCount', 'absentCount', 'esi', 'epf'];
  displayedColumns1: string[] = ['employeeName', 'employeeId', 'finalSalary', 'presentCount', 'absentCount', 'esi', 'epf'];
  years: any[] = getYears();
  months: Month[] = [
  ...
  getIndividulAtten() {
    ...
    if (form?.employeeId != '' && form?.month != '' && form?.year != '') {
      ...
      this.modelService.salaryIndiviual(obj).subscribe((res) => {
  ...
  getMonthlyAtten() {
    ...
    if (form?.districtId != '' && form?.month != '' && form?.year != '') {
      ...
      this.modelService.salary(obj).subscribe((res) => {
        this.dailyAtten = res?.responseModel
  ...
  downloadIndivitual() {
    if (this.type == 'Individual') {
      ...
      if (form?.employeeId != '' && form?.month != '' && form?.year != '') {
        ...
        this.getService.downloadMonthly('salary/get-daily-salary', obj).subscribe((data) => {
          switch (data.type) {
      ...
      if (form?.districtId != '' && form?.month != '' && form?.year != '') {
        ...
        this.getService.downloadMonthly('salary/get-monthly-salary', obj).subscribe((data) => {
          switch (data.type) {
            case HttpEventType.DownloadProgress:
              break;
            case HttpEventType.Response:
