...
export class ExpenseListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'expenseTitle','date','expenseDescription','amount','sender','receiver','transactiontype ','activeStatus'];
  ...
  getCheckedValue(): Expense[] {
    let transList: Expense[] = [];
  ...
  checkboxLabel(row?: Expense): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${this.expenseId + 1}`;
  }
  constructor(public config: ConfigService,public dialog: MatDialog,private getService:ExpenseService,private loader:AppLoaderService) { 
    this.config.init();
  ...
  getData(){
    ...
    this.getService.getExpense(obj).subscribe((data) => {
      this.paginationDTO.totalSize = data.pageSize;
      this.currentPageDataSize = data.responseModel.length;
      this.dataSource = new MatTableDataSource<Expense>(data.responseModel);
      this.dataSource.sort = this.sort;
    })
  }
  addSub() {
  ...
  action(option) {
    if (this.getCheckedValue().length == 1) {
      Swal.fire({
        ...
        if (result.value) {
          this.expense = this.getCheckedValue()[0];
          const dialogRef = this.dialog.open(ExpenseUpdateComponent, {
            disableClose: true,
            data: { payload: this.expense },
          });
          dialogRef.afterClosed().subscribe((result) => {
            if (result) {
              this.dataSource.sort = this.sort;
      ...
      Swal.fire({
        ...
        text: "Please Select One Expense!",
      });
    }
}
Delete(option){
...
let expenseId = [];
this.getCheckedValue().forEach(item => { expenseId.push(item.expenseId)
})
let obj={
  'customerId': this.customerId,
  'districtId': this.districtId,
...
this.getService.Delete(obj).subscribe((data) => {
  Swal.fire({
    icon: "success",
    title: "Deleted Expense Successfull",
    timer: 2500
  }).then(function(){
  ...
  if (this.getCheckedValue().length >= 1) {
    ...
    if (activeStatus == 2) {
      ...
      Swal.fire({
        ...
        text: 'Expense registration incompleted so can not ' + lblAction + '!',
      })
    }
      ...
      Swal.fire({
        icon: 'error',
        title: 'Expense Status',
        text: 'Please select the ' + opplblAction + ' status of the Expense!',
      });
    }
  } else {
    Swal.fire({
      icon: 'error',
      title: activeStatus == 2 ? 'Delete Expense' : 'Change Expense Status',
      text: 'Please Select Altleast One Expense!',
    });
  }
}
...
saveStatus(activeStatus) {
  let ids = [];
  this.getCheckedValue().forEach(item => { ids.push(item.expenseId)
  ...
  this.getService.status(obj).subscribe((res) => {
    if (res.statusCode == 200) {
      Swal.fire({
        icon: "success",
        title: " Expense Status Changed Successfull",
        timer: 2500
      }).then(function(){
        window.location.reload();
    });
      ...
      Swal.fire({
        icon: "error",
        title: " Expense Status Change Failed",
        timer: 2500
